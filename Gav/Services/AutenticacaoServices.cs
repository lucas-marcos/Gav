...
public class AutenticacaoServices : IAutenticacaoServices
{
    ...
    public ApplicationUser Autenticar(ApplicationUser usuario, string senha)
    {
        var usuarioCadastrado = _userManager.FindByEmailAsync(usuario.Email).Result
                                ...
                                ?? throw new GavException("Usuário ou senha inválida");
        ...
        if (!resultLogin.Succeeded) throw new GavException("Usuário ou senha inválida");
    ...
    public ApplicationUser CriarUsuario(ApplicationUser usuario, string senha)
    {
        ...
        if (result.Errors.Any(e => e.Code == "DuplicateUserName"))
            throw new GavException("O endereço de email já está sendo usado por outro usuário.");

        if (!result.Succeeded) throw new GavException("Encontramos os seguintes erros: " + string.Join(", ", result.Errors.Select(a => a.Description)));
